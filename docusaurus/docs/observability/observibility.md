...
![Kubernetes Dashboards](./assets/kubernetes-dashboards.webp)
...
![Application Metrics Dashboard](./assets/application-metrics.webp)
...
## 7. High Availability & Chaos Engineering

### Overview
Test EKS cluster resiliency through controlled failure scenarios using AWS Fault Injection Simulator (FIS) and Chaos Mesh.
...
**Step 3:** Scale UI deployment with topology spread
```bash
...
This scales to 5 replicas with pod anti-affinity rules

**Step 4:** Verify retail store accessibility
```bash
wait-for-lb $(kubectl get ingress -n ui -o jsonpath='{.items[0].status.loadBalancer.ingress[0].hostname}')
```

**Step 5:** Check pod distribution across AZs
```bash
~/$SCRIPT_DIR/get-pods-by-az.sh
```

Expected: Pods distributed across multiple availability zones

### 7.2 Pod Failure Simulation
...
Note pod ages and node assignments

**Step 2:** Simulate pod failure with Chaos Mesh
```bash
...
One pod should have recent age indicating Kubernetes auto-recovery

**Step 4:** Verify retail store availability
```bash
kubectl get ingress -n ui
```

Store should remain accessible during pod failure

### 7.3 Node Failure (Manual)
...
**Step 4:** Observe pod rescheduling
```bash
kubectl get pods -n ui -o wide
```

Pods should move to other available nodes
...
### 7.4 Node Failure with AWS FIS (Partial)

**Step 1:** Create FIS experiment template
- Define target: EC2 instances with specific tags
- Action: Stop 1 instance (partial failure)
- Duration: 5 minutes

**Step 2:** Run FIS experiment
```bash
...
```bash
watch kubectl get pods -n ui -o wide
```

Pods on failed node should reschedule to healthy nodes

**Step 4:** Check application availability
Monitor retail store URL - should remain accessible

**Step 5:** Review experiment results
- Check FIS Console for experiment status
- Analyze CloudWatch metrics during failure
- Review pod events: `kubectl describe pod <pod-name> -n ui`

### 7.5 Node Failure with AWS FIS (Complete)

**Step 1:** Create FIS experiment for all nodes in one AZ
- Target: All instances in specific AZ
- Action: Stop instances
- Stop condition: CloudWatch alarm threshold
...
**Step 3:** Monitor multi-pod rescheduling
```bash
watch kubectl get pods -n ui -o wide
```

All pods should migrate to nodes in other AZs

**Step 4:** Verify application resilience
Application should remain available if sufficient capacity exists

**Step 5:** Analyze recovery time
- Time to detect failure
- Time to reschedule pods
- Time for pods to become ready

### 7.6 Availability Zone Failure

**Step 1:** Setup multi-AZ deployment
Ensure pods are distributed across 3 AZs

**Step 2:** Verify current distribution
```bash
~/$SCRIPT_DIR/get-pods-by-az.sh
```

**Step 3:** Simulate AZ failure with FIS
- Target: All nodes in one availability zone
- Action: Stop instances
- Expected behavior: Pods migrate to remaining AZs

**Step 4:** Run AZ failure experiment
```bash
~/$SCRIPT_DIR/az-failure.sh
```

**Step 5:** Monitor pod migration
```bash
watch kubectl get pods -n ui -o wide
```

**Step 6:** Check pod distribution after failure
```bash
~/$SCRIPT_DIR/get-pods-by-az.sh
```

Pods should now be in 2 AZs only

**Step 7:** Verify application availability
- Check retail store URL
...
### 7.7 Create CloudWatch Synthetics Canary

**Step 1:** Create canary to monitor application
- Endpoint: Retail store URL
- Frequency: Every 1 minute
- Success criteria: HTTP 200 response

**Step 2:** Configure canary alerts
- Create CloudWatch alarm on canary failures
- SNS topic for notifications
- Threshold: 2 consecutive failures

**Step 3:** Use canary during chaos experiments
- Monitor canary metrics during FIS experiments
