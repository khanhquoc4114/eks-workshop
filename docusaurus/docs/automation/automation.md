...
#### Cluter bootstrap
1. Run pre-bootstrap checks to verify that everything is set up correctly. Run the following command for Flux CLI to perform the checks:

![](../img/automation/2026-02-03-23-49-22.png)

2. Now we can bootstrap Flux on our EKS cluster using a Gitea repository:

![](../img/automation/2026-02-03-23-51-22.png)

![](../img/automation/2026-02-03-23-51-39.png)

3. Now, let's verify that the bootstrap process completed successfully by running the following command:

![](../img/automation/2026-02-03-23-52-14.png)
#### Deploying an application
1. First let's remove the existing UI component so we can replace it:
Next, clone the repository we used to bootstrap Flux in the previous section:

![](../img/automation/2026-02-03-23-53-26.png)

2. Now, let's start populating the Flux repository by creating a directory for our "apps". This directory is designed to contain a sub-directory for each application component:
```bash
...
3. Copy this file to the Git repository directory:

![](../img/automation/2026-02-03-23-55-32.png)

4. Copy this file to the Git repository directory:

![](../img/automation/2026-02-03-23-55-50.png)

5. Copy the appropriate files to the Git repository directory:

![](../img/automation/2026-02-03-23-56-13.png)

![](../img/automation/2026-02-03-23-56-58.png)

6. Finally we can push our configuration to Git:

![](../img/automation/2026-02-03-23-57-31.png)

7. It will take Flux some time to notice the changes in Git and reconcile. 
You can use the Flux CLI to watch for our new apps kustomization to appear:

![](../img/automation/2026-02-04-00-01-58.png)

9. Once apps appears as indicated above use Ctrl+C to close the command. You should now have all the resources related to the UI services deployed once more. To verify, run the following commands:
![](../img/automation/2026-02-04-00-09-58.png)

10. Get the URL from the Ingress resource:
![](../img/automation/2026-02-04-00-10-33.png)

11. To wait until the load balancer has finished provisioning you can run this command:

![](../img/automation/2026-02-04-00-10-57.png)

12. And access it in your web browser. You will see the UI from the web store displayed and will be able to navigate around the site as a user.
...
#### Setting up Gitea
1. Let's install Gitea in our EKS cluster with Helm:
![](../img/automation/2026-02-04-00-38-53.png)
2. Make sure that Gitea is up and running before proceeding:
![](../img/automation/2026-02-04-00-44-25.png)
3. An SSH key will be needed to interact with Git. The environment preparation for this lab created one, we just need to register it with Gitea:
![](../img/automation/2026-02-04-00-44-45.png)
4. And we'll also need to create the Gitea repository that Argo CD will use:
![](../img/automation/2026-02-04-00-54-56.png)
5. Now we can set up an identity that Git will use for our commits:
![](../img/automation/2026-02-04-00-56-12.png)
6. And finally let's clone the repository and set up the initial structure:
![](../img/automation/2026-02-04-00-57-27.png)
#### Installing Argo CD
7. Let's begin by installing Argo CD in our cluster:
![](../img/automation/2026-02-04-00-59-52.png)
8. For the purpose of this lab, the Argo CD server UI has been configured to be accessible outside of the cluster using a Kubernetes service with a load balancer. To obtain the URL, execute the following commands:
![](../img/automation/2026-02-04-01-00-23.png)
9. The load balancer will take some time to provision. Use this command to wait until Argo CD responds:
![](../img/automation/2026-02-04-01-02-47.png)
10. For authentication, the default username is admin and the password is auto-generated. Retrieve the password with the following command:
![](../img/automation/2026-02-04-01-03-20.png)
11. Use the URL and credentials you just obtained to log in to the Argo CD UI.
![](../img/automation/2026-02-04-01-04-03.png)
![](../img/automation/2026-02-04-01-04-44.png)
12. To interact with Argo CD using the CLI, you need to authenticate with the Argo CD server:
![](../img/automation/2026-02-04-01-06-02.png)
13. Finally, register the Git repository with Argo CD to provide access:
![](../img/automation/2026-02-04-01-07-11.png)
#### Deploying an application
14. First, let's remove the existing sample application from the cluster:
![](../img/automation/2026-02-04-01-08-20.png)
15. Let's copy this file to our Git directory:
![](../img/automation/2026-02-04-01-09-12.png)
16. Now we'll push our configuration to the Git repository:
![](../img/automation/2026-02-04-01-09-41.png)
17. Next, let's create an Argo CD Application configured to use our Git repository:
![](../img/automation/2026-02-04-01-12-22.png)
18. We can verify that the application has been created:
![](../img/automation/2026-02-04-01-11-25.png)
19. This application is now visible in the Argo CD UI:
![](../img/automation/2026-02-04-01-12-59.png)
![](../img/automation/2026-02-04-01-16-36.png)
20. In Argo CD, "out of sync" indicates that the desired state defined in your Git repository doesn't match the actual state in your Kubernetes cluster. Although Argo CD is capable of automated synchronization, for now we'll manually trigger this process:
![](../img/automation/2026-02-04-01-17-28.png)
![](../img/automation/2026-02-04-01-17-44.png)
21. After a short period, the application should reach the Synced state, with all resources deployed. The UI should look like this:
![](../img/automation/2026-02-04-01-17-59.png)
22. To verify that all resources related to the UI service are deployed, run the following commands:
![](../img/automation/2026-02-04-01-18-45.png)
#### Updating an application
23. Let's copy this configuration file to our Git repository directory:
![](../img/automation/2026-02-04-01-19-29.png)
24. Now we'll commit and push our changes to the Git repository:
![](../img/automation/2026-02-04-01-20-19.png)
25. At this point, Argo CD will detect that the application state in the repository has changed. You can either click Refresh and then Sync in the Argo CD UI, or use the argocd CLI to synchronize the application:
![](../img/automation/2026-02-04-01-21-00.png)
![](../img/automation/2026-02-04-01-21-28.png)
26. After synchronization completes, the UI deployment should now have 3 pods running:
![](../img/automation/2026-02-04-01-21-52.png)
27. To verify that our update was successful, let's check the deployment and pod status:
![](../img/automation/2026-02-04-01-22-21.png)


#### App of app
##### Setup
28. First, let's copy this foundational App of Apps configuration to our Git directory:
![](../img/automation/2026-02-04-01-26-33.png)
29. Now, let's commit and push these changes to the Git repository:
![](../img/automation/2026-02-04-01-27-11.png)
30. Next, we need to create a new Argo CD Application to implement the App of Apps pattern. While doing this, we'll enable Argo CD to automatically synchronize the state in the cluster with the configuration in the Git repository using the --sync-policy automated flag:
![](../img/automation/2026-02-04-01-27-58.png)
![](../img/automation/2026-02-04-01-28-23.png)
##### Adding more workloads 
31. Let's copy the application chart files to our Git repository directory:
![](../img/automation/2026-02-04-01-29-19.png)
32. Next, commit and push these changes to the Git repository:
![](../img/automation/2026-02-04-01-29-54.png)
33. Sync the apps application:
![](../img/automation/2026-02-04-01-30-32.png)
![](../img/automation/2026-02-04-01-30-49.png)
34. When Argo CD completes the process, all our applications will be in the Synced state as shown in the Argo CD UI:
![](../img/automation/2026-02-04-01-31-55.png)
35. We should now see a set of new namespaces with each application component deployed:
![](../img/automation/2026-02-04-01-32-15.png)
36. Let's examine one of the deployed workloads more closely. For example, we can check the carts component:
![](../img/automation/2026-02-04-01-32-36.png)

## 2. Control Planes
### 2.1 AWS Controllers for Kubernetes (ACK)
...
![](../img/automation/2026-02-06-04-49-01.png)
...
![](../img/automation/2026-02-06-04-50-18.png)
...
![](../img/automation/2026-02-06-04-51-06.png)
...
```
![](../img/automation/2026-02-06-04-52-24.png)

**Step 3:** Verify table status
```bash
kubectl get tables.dynamodb.aws.upbound.io
```
![](../img/automation/2026-02-06-04-52-41.png)
...
kubectl kustomize ~/environment/eks-workshop/modules/automation/controlplanes/crossplane/app \
  | envsubst | kubectl apply -f-
```
![](../img/automation/2026-02-06-04-53-24.png)
...
kubectl rollout status -n carts deployment/carts --timeout=40s
```
![](../img/automation/2026-02-06-04-56-01.png)
...
wait-for-lb $(kubectl get service -n ui ui-nlb -o jsonpath="{.status.loadBalancer.ingress[*].hostname}{'\n'}")
```
![](../img/automation/2026-02-06-04-59-53.png)

- Open browser
![](../img/automation/2026-02-06-05-00-36.png)
- Adding a few items to the cart
![](../img/automation/2026-02-06-05-01-46.png)
- check if items are present in the DynamoDB table as well, run:
```bash
aws dynamodb scan --table-name "${EKS_CLUSTER_NAME}-carts-crossplane"
```
![](../img/automation/2026-02-06-05-02-20.png)
#### Compositions
...
```
![](../img/automation/2026-02-06-05-03-06.png)
...
```
![](../img/automation/2026-02-06-05-04-00.png)
...
kubectl wait dynamodbtables.awsblueprints.io ${EKS_CLUSTER_NAME}-carts-crossplane -n carts \
  --for=condition=Ready --timeout=5m
```
![](../img/automation/2026-02-06-05-05-48.png)
**Step 3:** Verify resources created
![](../img/automation/2026-02-06-05-05-04.png)
```bash
kubectl get table
...
![](../img/automation/2026-02-06-05-06-27.png)
**Step 4:** Test application and verify data in DynamoDB
1. First, we need to restart the pods to ensure they're using the updated configuration:
![](../img/automation/2026-02-06-05-08-18.png)
2. To access the application, we'll use the same load balancer as in the previous section. Let's retrieve its hostname:
![](../img/automation/2026-02-06-05-08-39.png)
3. access the application by copying this URL into your web browser
![](../img/automation/2026-02-06-05-09-01.png)
4. Add a few items to your cart in the web interface.
![](../img/automation/2026-02-06-05-09-30.png)
5. confirm that these items are being stored in the DynamoDB table
```bash
aws dynamodb scan --table-name "${EKS_CLUSTER_NAME}-carts-crossplane"
```
![](../img/automation/2026-02-06-05-10-02.png)
